...
con_path=newproba(con);

%calculating betweenness centrality
bet=betweenness_w(con_path);
bet=bet';

%calculating bridging coefficient
brd=bridging(con);

%calculating betweenness centrality by multiplying each element of
%betweenness centrality with each element of bridging coefficients
brd=bet.*brd;

%saving betweenness centrality with some of the other identifiers
id=[nyear, nseason, nsim, npldclass];

%save the bridging centrality values as a matrix
brd_all=[brd_all; id, brd];
end
end
end
end
...
writematrix(brd_all,'brd_mat.csv') 